...
class CameraPublisher(Node):
    ...
    def __init__(self):
        ...
        self.publisher_src_xy= self.create_publisher(std_msgs.msg.Float32MultiArray, 'camera/src_xy', 10)
    ...
    def image_traitement(self,frame):
        ...
        # Define the color range for detecting red in HSV space
        # Lower and upper bounds for the red color in HSV (two ranges for full red spectrum)
        lower_red_1 = np.array([0, 120, 70])  # Lower bound for red
        upper_red_1 = np.array([10, 255, 255])  # Upper bound for red
        lower_red_2 = np.array([170, 120, 70])  # Lower bound for red (another range)
        upper_red_2 = np.array([180, 255, 255])  # Upper bound for red (another range)

        # Create binary masks for detecting red in both HSV ranges
        mask1 = cv2.inRange(hsv, lower_red_1, upper_red_1)
        mask2 = cv2.inRange(hsv, lower_red_2, upper_red_2)

        # Combine both masks to detect all red areas
        mask = cv2.bitwise_or(mask1, mask2)

        # Calculate moments to get the barycenter of the detected red color
        moments = cv2.moments(mask)

        if moments["m00"] != 0:  # If the mask has detected red
            # Calculate the center of mass (barycenter)
            cx = int(moments["m10"] / moments["m00"])
            cy = int(moments["m01"] / moments["m00"])
            coordinates = msg.Float32MultiArray([cx,cy])
            self.publisher_src_xy.publish(coordinates)
            print()
            # Draw the barycenter (centroid) as a red dot on the frame
            cv2.circle(frame, (cx, cy), 5, (0, 0, 255), -1)
            cv2.putText(frame, f"Barycenter: ({cx}, {cy})", (cx + 10, cy),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        else:
            # Display message if no red color is detected
            cv2.putText(frame, "Red color not detected", (50, 50),
